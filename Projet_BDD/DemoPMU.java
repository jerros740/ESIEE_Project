...
public class DemoPMU {
	...
	String query="CALL rosj2.inscrire('"+nom+"','"+prenom+"','"+login+"','"+passwd+"')";
	st.execute(query);
	...
	ResultSet rset= st.executeQuery("SELECT rosj2.connexion('"+login+"','"+pwd+"') FROM dual");
	if (rset.next())
	    ok= (rset.getInt(1) == 1);
	if (!ok) {
    ...
    {
	Statement st= cnx.createStatement();
	ResultSet rset= st.executeQuery("SELECT id_ticket,nb_chevaux,desordre,montant,etat FROM TABLE(rosj2.mes_tickets('"+login+"','"+pwd+"'))");
	...
	//
	ResultSet res= st.executeQuery("SELECT * FROM rosj2.non_courues");
	while (res.next())
	...
	if (course > 0)
		...
		res=st.executeQuery("SELECT * FROM rosj2.partants WHERE id_course="+course);
		while (res.next())
		...
		if (sc.nextLine().equals("o"))
			...
			res= st.executeQuery("SELECT rosj2.creer_ticket('"+login+"','"+pwd+"',"+nbchevaux+","+desordre+","+montant+") FROM dual");
			...
			if (res.next() && (id_ticket= res.getInt(1)) > 0)
				...
				for (int i=1; i <= nbchevaux; i++)
					...
					String query= "CALL rosj2.ajout_mise_ticket('"+login+"','"+pwd+"',"+id_ticket+","+course+","+d+","+i+")";
					st.execute(query);
	...
	cnx = DriverManager.getConnection("jdbc:oracle:thin:@mvx1.esiee.fr:1521:xe", "rosj2", "xjoza18121999");
